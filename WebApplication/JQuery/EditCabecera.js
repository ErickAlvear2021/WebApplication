$(document).ready(function () {
    ...
    var _id, _checked, _nuevoestado, _descripcion, _objeto, _objIndex;
    ...
    $('#btnAgregar').click(function () {
        if ($.trim($('#txtDetalle').val()).length == 0) {
            ...
            alertify.notify('ingrese un detalle..!', 'warning ', 5, function () { console.log('dismissed'); });
        ...
        if ($.trim($('#txtValorV').val()).length == 0 && $.trim($('#txtValorI').val()).length == 0) {
            ...
            alertify.notify('ingrese valor texto o entero..!', 'warning ', 5, function () { console.log('dismissed'); });
            return;
        }

        if ($.trim($('#txtValorV').val()).length > 0 && $.trim($('#txtValorI').val()).length > 0) {
         
            alertify.notify('solo valor texto o entero..?', 'error ', 5, function () { console.log('dismissed'); });
            return;
        }
        ...
        if (_tipoSave == 'add') {
            $.each(_result, function (i, item) {
                if (item.ArryPadeNombre.toUpperCase() == _descripcion.toUpperCase()) {
                     ...
                     alertify.notify('parametro ya existe..!', 'error ', 5, function () { console.log('dismissed'); });
                    _continuar = false;
                    return false;
                }
                else {
                    $.each(_result, function (i, item) {
                        if (_valori == 0) {
                            if (item.ArryPadeValorV.toUpperCase() == _valorv.toUpperCase()) {
                                 ...
                                 alertify.notify('valor texto ya existe..!', 'error ', 5, function () { console.log('dismissed'); });
                                _continuar = false;
                                return false;
                            } else {
                                _continuar = true;
                            ...
                            if (item.ArryPadeValorI == _valori) {
                                 ...
                                 alertify.notify('valor entero ya existe..!', 'error ', 5, function () { console.log('dismissed'); });
                                _continuar = false;
                                return false;
                            } else {
                                _continuar = true;
    ...
    $("#btnSave").click(function (eve) {
        ...
        if (_nomparametro == '') {
             ...
             alertify.notify('nombre del parametro..!', 'warning ', 5, function () { console.log('dismissed'); });
            return;
        }
        ...
        $.ajax({
            ...
            success: function (datos) {
                if (datos.success == true) {
                    ...
                    alertify.notify('nombre del parametro ya exixte..!', 'error ', 5, function () { console.log('dismissed'); });
                }
            },
            error: function (error) {
                console.log(error);
